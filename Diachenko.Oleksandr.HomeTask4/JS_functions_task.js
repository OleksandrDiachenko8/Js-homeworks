...
// 1. Вивести в в html імя країни та список всіх мов, якими розмовляють в ній. Також вивести імя країни,
// перекладене іспанською та французькою мовами у форматі:
// Translations:       
// 	ES - Argelia
//     FR - Algérie

           //  Selected country 
function selectedCountryOutput(array, countryName) {
    countryEl = array.find(function(el){return el.name==countryName;});

    const countryBlock = document.createElement('div');
    countryBlock.className = 'country';

    const countryNameTitle = document.createElement('h3');
    countryNameTitle.innerText = countryEl.name;
    const translationsTitle = document.createElement('p');
    translationsTitle.innerText = 'Translations:';
    const translateEsFrBlock = document.createElement('div');
    translateEsFrBlock.innerHTML = `<span>ES - ${countryEl.translations.es}</span><br><span style="margin-left: 10px">FR - ${countryEl.translations.fr}</span>`;
    const speakingLangs = document.createElement('p');
    let languages = countryEl.languages.map(function(arr){return arr.name});
    speakingLangs.innerText = `${countryEl.name} peoples speaking languages: ${languages}.`;
    
    countryBlock.appendChild(countryNameTitle);
    countryBlock.appendChild(translationsTitle);
    countryBlock.appendChild(translateEsFrBlock);
    countryBlock.appendChild(speakingLangs);

    document.body.appendChild(countryBlock);
}

selectedCountryOutput(countries, 'Montenegro');


                //  All countries 
for (let i = 0; i < countries.length; i++) {
    
    const eachCountryBlock = document.createElement('div');
    eachCountryBlock.className = 'country';

    const countryNameTitle = document.createElement('h3');
    countryNameTitle.innerText =`${i+1}. ${countries[i].name}`;

    const translationsTitle = document.createElement('p');
    translationsTitle.innerText = 'Translations:';
    const translateEsFrBlock = document.createElement('div');
    translateEsFrBlock.innerHTML = `<span>ES - ${countries[i].translations.es}</span><br><span style="margin-left: 10px">FR - ${countries[i].translations.fr}</span>`;

    const speakingLangs = document.createElement('p');
    let languages = countries[i].languages.map(function(arr){return arr.name})
    speakingLangs.innerText = `${countries[i].name} peoples speaking languages: ${languages}.`
    

    eachCountryBlock.appendChild(countryNameTitle);
    eachCountryBlock.appendChild(translationsTitle);
    eachCountryBlock.appendChild(translateEsFrBlock);
    eachCountryBlock.appendChild(speakingLangs);
 
    document.body.appendChild(eachCountryBlock);
...
// 2. Написати функцію, яка приймає обєкт з трьома властивостями типу number, які виступають сторонами трикутника.
// Назви ключів обєкту можуть мінятись. Для прикладу, ми можемо отримати аргументом або {a: 17, b: 30, c:30} або {s: 17, m: 30, n:30}
// Функція повинна вираховувати периметр трикутника.

function triangle_perimeter(obj) {
    let perimeter = 0;
...
// 3. Написати функцію, яка переводить обєкт з попереднього прикладу у масив:
// 	{a: 17, b: 30, c:30} -> [17, 30, 30]
...
// 4. Написати функцію, яка приймає на вхід масив країн і вміє вирахувати масив регіонів, до яких входять ці країни.
// В масиві регіони не повинні повторюватись.

function RegionsParse(arr) {
    let RegionsArray = [];
    for (let i = 0; i < arr.length; i++){
        if (!RegionsArray.includes(arr[i].region) & (arr[i].region.length > 0)) {
            RegionsArray.push(arr[i].region);
        }
    }
    return RegionsArray;
}
...
let triangle_one = {a: 17, b: 30, c:30};
let triangle_two = {s: 17, m: 30, n:30};
console.log("Периметр трикутника №1:", triangle_perimeter(triangle_one));
console.log("Периметр трикутника №2:", triangle_perimeter(triangle_two));

console.log("Массив сторін трикутника:\n", objToArray(triangle_two));

console.log("Массив регіонів до яких входять країни:\n", RegionsParse(countries));
